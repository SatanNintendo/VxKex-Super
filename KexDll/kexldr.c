        ...
        IN      ULONG                           Flags)
{
        ...
        if (!NT_SUCCESS(Status)) {
                ...
                //
                // FALLBACK: If D3DKMTOpenAdapterFromLuid or D3DKMTEnumAdapters2
                // was not found in the requested DLL (typically gdi32.dll on Win7),
                // try to load it from KxDx.dll which provides Win10+ D3DKMT
                // implementations that work on Win7 via D3DKMTOpenAdapterFromHdc.
                //
                // This is critical for Vulkan applications that use third-party
                // overlay layers (e.g. Mirillis Action!) which call these
                // functions via GetProcAddress without checking the return value,
                // leading to NULL pointer dereference crashes.
                //

                if (ProcedureName != NULL && NT_SUCCESS(Status2)) {
                        STATIC CONST struct {
                                PCSTR   Name;
                        } D3DKMTFallbackProcs[] = {
                        ...
                        for (Index = 0; Index < ARRAYSIZE(D3DKMTFallbackProcs); Index++) {
                                ANSI_STRING FallbackProcName;

                                RtlInitAnsiString(&FallbackProcName, D3DKMTFallbackProcs[Index].Name);

                                if (RtlEqualString(ProcedureName, &FallbackProcName, FALSE)) {
                                        PVOID KxDxBase;
                                        UNICODE_STRING KxDxName;

                                        RtlInitConstantUnicodeString(&KxDxName, L"kxdx.dll");

                                        Status2 = LdrGetDllHandleEx(
                                                LDR_GET_DLL_HANDLE_EX_UNCHANGED_REFCOUNT,
                                                NULL,
                                                NULL,
                                                &KxDxName,
                                        ...
                                        if (!NT_SUCCESS(Status2)) {
                                                //
                                                // KxDx is not loaded yet. Load it now.
                                                //
                                        ...
                                        if (NT_SUCCESS(Status2) && KxDxBase != NULL) {
                                                NTSTATUS Status3;

                                                Status3 = LdrGetProcedureAddressEx(
                                                        KxDxBase,
                                                        ProcedureName,
                                                        ProcedureNumber,
                                                        ProcedureAddress,
                                                ...
                                                if (NT_SUCCESS(Status3)) {
                                                        KexLogInformationEvent(
                                                                L"Redirected D3DKMT procedure %hZ from %wZ to kxdx.dll",
                                                                ProcedureName,
                                                                &BaseDllName);
