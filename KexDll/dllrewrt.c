        ...
        IN      PCUNICODE_STRING        FullDllName)
{
        ...
        unless (KexData->IfeoParameters.DisableAppSpecific) {
                ...
                //
                // APPSPECIFICHACK: Mirillis Action! Vulkan overlay layer.
                // This DLL calls D3DKMTOpenAdapterFromLuid (Win10+ API in gdi32.dll)
                // and doesn't handle the failure case properly on Win7. While we
                // provide D3DKMTOpenAdapterFromLuid through KxDx (via GetProcAddress
                // fallback), we also exclude this DLL from import rewriting to prevent
                // any other dxgi-related crashes, consistent with the action_x64/x86
                // exclusion above.
                //

                RtlInitConstantUnicodeString(&TargetDllName, L"MirillisActionVulkanLayer.dll");
